...
public class MyBot {
    ...
    public static void main(final String[] args) {
        // map to hold the turn number in which the ships have to start depositing, according to map size
        // disregarding the collected halite amount
        final Map<Integer, Integer> LATE_GAME = new HashMap<Integer, Integer>() {
            {
                put(new Integer(64), 370);
                put(new Integer(80), 400);
                put(new Integer(112), 445);
            }
        };

        // minimum distance to shipyard where the dropoff has to be created (if the map has large halite amounts)
        // according to map size
        final Map<Integer, Integer> MINIM_DISTANCE = new HashMap<Integer, Integer>() {
            {
                put(new Integer(64), 10);
                put(new Integer(80), 15);
                put(new Integer(112), 25);
            }
        };
        ...
        final int HALITE_FOR_DROPOFF = 15000;

        // minimum number of ships in an area necesarry to create dropoff
        final int SHIPS_FOR_DROPOFF = 3;
        ...
        for (;;) {
            ...
            for (final Ship ship : me.ships.values()) {
                ...
                if (!shipState.keySet().contains(ship.id.id)) {
                    ...
                    if (ship.halite >= Constants.MAX_HALITE || game.turnNumber >= LATE_GAME.get(gameMap.width + gameMap.height)) {
                ...
                if (shipState.get(ship.id.id).equals('c')) {
                    ...
                    if (!droppoff  && me.halite >= Constants.DROPOFF_COST &&
                            gameMap.calculateDistance(ship.position, me.shipyard.position) > MINIM_DISTANCE.get(gameMap.width + gameMap.height) &&
                        ...
                        // let the ships know they have to collide the ship on the shipyard/dropoff, disregaring
                        // if on the structure there is an enemy or friendly ship
                        boolean late = false;
                        if (game.turnNumber >= LATE_GAME.get(gameMap.width + gameMap.height)) {
                            late = true;
                        }
                        Direction dir = gameMap.naiveNavigate(ship, p, me, late);
